---
title: 알림 수신 시간대 설정해보기
date: 2026-05-08T20:00+09:00
description: 알림 수신 요일·시간대 설정 기능을 구현하며 마주친 비트마스크 표현, 한국 달력의 일요일 시작 관행, 라이브러리·표준별 DayOfWeek 차이를 정리합니다.
tags:
    - Bitmask
    - DayOfWeek
    - PostgreSQL
...
![알림 수신 시간대 설정 UI 목업 — 요일 토글과 시작/종료 시각 입력](/images/posts/dayofweek-bitmask/001.svg)

사용자가 보게 될 결과물은 `dayOfWeekMask` 와 `startTime`/`endTime` 두 시각 필드, 그리고 `weekStart` 가 결합된 토글 UI 로 단순합니다. 이 단순함을 유지하기 위해 어떤 결정들이 오갔는지는 사용자 화면 너머에 남아요.

## 참고 자료

- [MDN — Date.prototype.getDay()](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Global_Objects/Date/getDay)
- [Day.js — `.day()` / `.isoWeekday()`](https://day.js.org/docs/en/get-set/day)
- [PostgreSQL — `EXTRACT` (`dow`, `isodow`)](https://www.postgresql.org/docs/current/functions-datetime.html#FUNCTIONS-DATETIME-EXTRACT)
- [Java — `java.time.DayOfWeek`](https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/time/DayOfWeek.html)
