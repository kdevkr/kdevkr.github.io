...
런타임 정비를 마친 뒤에는 ==워크플로우 권한== 도 같이 손봤어요. 기존 워크플로우에는 `permissions:` 블록이 없어 저장소 디폴트 권한이 그대로 잡에 부여되고 있었고, `gh-pages` 푸시는 ==5년 전에 만든 클래식 PAT== (`GP_TOKEN`)에 의존하고 있었거든요. 클래식 PAT 는 저장소 단위 스코프 제한이 없어, `repo` 스코프 하나로 ==계정이 접근 가능한 모든 저장소에 쓰기 권한== 을 갖습니다. 단일 블로그 배포에 쓰기엔 명백히 과한 권한이었고, 오래된 토큰이 조용히 살아 있다는 사실 자체가 공격면이었어요.
...
- **`permissions: contents: write` 명시**: 잡이 받는 토큰의 스코프를 `gh-pages` 푸시에 필요한 콘텐츠 쓰기 권한으로만 좁혔어요. 그 외 issues/packages/actions 스코프는 자동으로 `none` 이 됩니다.
- **PAT → `GITHUB_TOKEN` 전환**: `peaceiris/actions-gh-pages` 는 같은 저장소의 `gh-pages` 브랜치라면 잡 시작 시 자동 발급되는 `GITHUB_TOKEN` 만으로 푸시할 수 있어요. ==PAT 의 만료·로테이션 부담이 사라지고==, 잡이 끝나면 토큰이 자동 폐기되어 유출 윈도우가 분 단위로 줄어듭니다.

`GITHUB_TOKEN` 으로 갈아탈 때 한 가지 주의할 점은, ==이 토큰으로 일으킨 푸시는 후속 워크플로우를 트리거하지 않는다== 는 거예요. 봇 푸시가 워크플로우를 또 깨우는 무한 루프를 막기 위한 GitHub 의 의도된 동작입니다. `gh-pages` 브랜치 푸시를 트리거로 듣는 다른 워크플로우가 있다면 PAT 를 유지해야 해요. 이 블로그는 해당 사항이 없어 전환했어요.
