...
먼저, `폴트(Fault)`는 설비 또는 시스템에서 발생한  <u>오류 또는 고장</u>을 말합니다. 일반적으로 폴트 항목에는 오류 또는 장애 이외의 항목이 포함되는데 일부 설비에서 시스템의 고장을 일으키는 상황을 보호하기 위해서 동작하는 차단기 또는 릴레이와 같은 장치가 동작한 상태를 `트립(Trip)`이라고 합니다. Trip은 설비에 따라서 과부하, 과•저전압, 과•저전류, 과열 센서 검출, 지락(누전) 등이 있습니다. 또한, 폴트에는 트립 이외에 경고(Warning) 상태인 경보(Alarm)도 포함하고 있습니다.
