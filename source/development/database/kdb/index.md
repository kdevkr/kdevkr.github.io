---
    title: KDB+
    description: The World's Fastest Time-Series Database
    comments: false
---

![](https://www.intel.in/content/dam/www/public/us/en/images/logos/4x3/kx-logo-4x3.png.rendition.intel.web.480.360.png)

#### KDB 튜토리얼

- [설치 및 접속](/archives/kdb-tutorial/install-and-connection)
- [변수 할당 및 테이블 생성](/archives/kdb-tutorial/assign-variables-and-tables)
- [Q-SQL](/archives/kdb-tutorial/q-sql)
- [그룹핑과 집계](/archives/kdb-tutorial/grouping-and-aggregation)
- [파일 다루기](/archives/kdb-tutorial/file)
- [네임스페이스](/archives/kdb-tutorial/namespace)
- [함수](/archives/kdb-tutorial/function)
- [자바 클라이언트](/archives/kdb-tutorial/java-client)


#### Libraries

- [Kdb+ Database Examples](https://github.com/timeseries/kdb)  
- [Q Tips: Fast, Scalable and Maintainable Kdb+](https://github.com/psaris/qtips)  
- [kdb+ Core Libraries and Utilities](https://github.com/BuaBook/kdb-common)


#### 참고
- [Kdb+ and q documentation](https://code.kx.com/q/)  
- [Java API for kdb+](https://code.kx.com/q/wp/java-api/) 